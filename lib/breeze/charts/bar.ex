defmodule Breeze.Charts.Bar do
      ...
      rows =
        if plot_height < 1 or group_width < bar_count * 2 - 1 do
          ...
          plot =
            for bottom <- (plot_height - 1)..0//-1 do
              ...
              [
                ...
                TextSpan.new("│", %{foreground_color: :border})
              ] ++
          ...
          plot ++
            [
              [
                TextSpan.new(String.pad_leading("0", label_width)),
                TextSpan.new("└" <> String.duplicate("─", plot_width), %{
  ...
  defp validate!(series, width, mode, max_value) do
    ...
    unless mode in ["grouped", "stacked"] do
      raise ArgumentError, ~s(bar_chart mode must be "grouped" or "stacked")
    end
  ...
  defp legend(series, width) do
    {lines, current, _used} =
      Enum.reduce(series, {[], [], 0}, fn item, {lines, current, used} ->
        ...
        spans = Text.legend_spans(text, item.color)
        ...
        cond do
          ...
          used + 2 + size <= width ->
            {lines, current ++ [TextSpan.new("  ")] ++ spans, used + 2 + size}

          true ->
            {[current | lines], spans, size}
        end
      end)
