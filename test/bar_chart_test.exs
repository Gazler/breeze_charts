defmodule Breeze.Charts.BarChartTest do
  ...
  test "public block switches orientation and preserves explicit vertical dimensions when empty" do
    ...
    {_, horizontal} =
      Breeze.Renderer.render(ChartView, %{assigns | orientation: "horizontal"},
        terminal: terminal
      )
  ...
  test "shared data selects values by key in data and series order with optional colors" do
    ...
    assert {["Y", "X"],
            [
              %{key: "b", name: "B", color: :primary, values: [-2, 3]},
    ...
    assert {["X", "Y"],
            [
              ...
              %{key: "b", values: [3, -2], color: :secondary}
